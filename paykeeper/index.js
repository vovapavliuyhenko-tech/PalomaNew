...
function allowedRange(id) {
  /* Строка корзины — «<id товара>-<размер>»: c1-M, m9-XL.
     Каталожные id начинаются с c… (букеты, вазы) и m… (моно-букеты);
     кофе использует menu-…, поэтому пересечений нет. Если id похож на
     каталожный, но товара нет — это ошибка, а не «свободная сумма». */
  const code = String(id || "").match(/^([cm]\d+)(?:[-_]|$)/);
  if (code) {
